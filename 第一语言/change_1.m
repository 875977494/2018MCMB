function check = change_1(check,p_life,p_out,p_died,p_e,aa)

bl = sum(check(:,:) == -1)/(sum(check(:,:) >= 1)+(sum(check(:,:) == 0.1)));
p_died = p_died * bl;
p_out = p_out * bl;
...
bili = sum(check(:) == 0.1)/(sum(check(:) == 0.1)+sum(check(:) == 1));  % jisuan bai/zongshu 
s = rand(length(ss16),1);
s1 = find(s <= bili);
s2 = find(s > bili);

[c,d] = find(check(:,:) == 0.1); %baisekuai qianchu huozhe siwang 
l = length(c);
ss = rand(l,1);
...
[e,f] = find(check(:,:) >= 1 & check(:,:) ~= 10.1);  %yansekuai siwang huozhe qianchu 
l = length(e);
sss = rand(l,1);
