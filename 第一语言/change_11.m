function check = change_11(check,p_life,p_out,p_died,p_e,aa)

%4 0.4 -4
aa = aa(:,11);

[a,b] = find(check(:,:) == -11); %xuanchusiren
...
bili = sum(check(:) == 0.11)/(sum(check(:) == 0.11)+sum(check(:) == 11));  % jisuan bai/zongshu 
...
for i = 1:length(s3)
    check(c(s3(i)),d(s3(i))) = -11;
end

for i = 1:length(s4)
    check(c(s4(i)),d(s4(i))) = -11;
end

for i = 1:length(s5)
    check(c(s5(i)),d(s5(i))) = 11;
end

for i = 1:length(s6)
    check(e(s6(i)),f(s6(i))) = -11;
end

for i = 1:length(s7)
    check(e(s7(i)),f(s7(i))) = -11;
end
