function check = change_15(check,p_life,p_out,p_died,p_e,aa)

%4 0.4 -4
aa = aa(:,15);
[a,b] = find(check(:,:) == -15); %xuanchusiren
...
bili = sum(check(:) == 0.15)/(sum(check(:) == 0.15)+sum(check(:) == 15));  % jisuan bai/zongshu 
...
for i = 1:length(s3)
    check(c(s3(i)),d(s3(i))) = -15;
end

for i = 1:length(s4)
    check(c(s4(i)),d(s4(i))) = -15;
end

for i = 1:length(s5)
    check(c(s5(i)),d(s5(i))) = 15;
end

for i = 1:length(s6)
    check(e(s6(i)),f(s6(i))) = -15;
end

for i = 1:length(s7)
    check(e(s7(i)),f(s7(i))) = -15;
end
