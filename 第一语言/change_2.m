function check = change_2(check,p_life,p_out,p_died,p_e,aa)

aa = aa(:,2);
%2 0.2 -2 brown white grey 

[a,b] = find(check(:,:) == -2); %xuanchusiren
...
bili = sum(check(:) == 0.2)/(sum(check(:) == 0.2)+sum(check(:) == 2));  % jisuan bai/zongshu 
...
for i = 1:length(s3)
    check(c(s3(i)),d(s3(i))) = -2;
end

for i = 1:length(s4)
    check(c(s4(i)),d(s4(i))) = -2;
end

for i = 1:length(s5)
    check(c(s5(i)),d(s5(i))) = 2;
end

for i = 1:length(s6)
    check(e(s6(i)),f(s6(i))) = -2;
end

for i = 1:length(s7)
    check(e(s7(i)),f(s7(i))) = -2;
end
