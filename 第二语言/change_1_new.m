function [check,newcheck] = change_1_new(check,p_life,p_out,p_died,aa,ab)
 
newcheck = check(1:52,1:52);
aa = aa(:,1);
ab = ab(:,1);
...
[c,d] = find(newcheck == -1); %xuanchusiren
l = length(c);
s = rand(l,1);
ss1 = find(s <= aa(1,:));
...
%% kaishixunhaun 
for i = 1:length(s1)
    newcheck(a(s1(i)),b(s1(i))) = -1;
end
 
for i = 1:length(s2)
    newcheck(a(s2(i)),b(s2(i))) = -1;
end
...
for i = 1:length(ss16)
    newcheck(c(ss16(i)),d(ss16(i))) = 0.1;
end
...
for i = 1:length(sss16)
    newcheck(e(sss16(i)),f(sss16(i))) = -1;
end
 
for i = 1:length(sss17)
    newcheck(e(sss17(i)),f(sss17(i))) = -1;
end

check(1:52,1:52) = newcheck;
