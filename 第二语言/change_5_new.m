function [check,newcheck] = change_5_new(check,p_life,p_out,p_died,aa,ab)
 
newcheck = check(101:130,1:30);
aa = aa(:,5);
ab = ab(:,5);
...
%% kaishixunhaun 
for i = 1:length(s1)
    newcheck(a(s1(i)),b(s1(i))) = -5;
end
 
for i = 1:length(s2)
    newcheck(a(s2(i)),b(s2(i))) = -5;
end
...
for i = 1:length(ss16)
    newcheck(c(ss16(i)),d(ss16(i))) = 0.5;
end
...
for i = 1:length(sss16)
    newcheck(e(sss16(i)),f(sss16(i))) = -5;
end
 
for i = 1:length(sss17)
    newcheck(e(sss17(i)),f(sss17(i))) = -5;
end

check(101:130,1:30) = newcheck;
