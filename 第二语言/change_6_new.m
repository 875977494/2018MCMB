function [check,newcheck] = change_6_new(check,p_life,p_out,p_died,aa,ab)
 
newcheck = check(101:120,30:51);
aa = aa(:,6);
ab = ab(:,6);
...
%% kaishixunhaun 
for i = 1:length(s1)
    newcheck(a(s1(i)),b(s1(i))) = -6;
end
 
for i = 1:length(s2)
    newcheck(a(s2(i)),b(s2(i))) = -6;
end
...
for i = 1:length(ss16)
    newcheck(c(ss16(i)),d(ss16(i))) = 0.6;
end
...
for i = 1:length(sss16)
    newcheck(e(sss16(i)),f(sss16(i))) = -6;
end
 
for i = 1:length(sss17)
    newcheck(e(sss17(i)),f(sss17(i))) = -6;
end

check(101:120,30:51) = newcheck;
