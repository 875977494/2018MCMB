function [check,newcheck] = change_9_new(check,p_life,p_out,p_died,aa,ab)
 
newcheck = check(104:118,58:72);
aa = aa(:,9);
ab = ab(:,9);
...
%% kaishixunhaun 
for i = 1:length(s1)
    newcheck(a(s1(i)),b(s1(i))) = -9;
end
 
for i = 1:length(s2)
    newcheck(a(s2(i)),b(s2(i))) = -9;
end
...
for i = 1:length(ss16)
    newcheck(c(ss16(i)),d(ss16(i))) = 0.9;
end
...
for i = 1:length(sss16)
    newcheck(e(sss16(i)),f(sss16(i))) = -9;
end
 
for i = 1:length(sss17)
    newcheck(e(sss17(i)),f(sss17(i))) = -9;
end

check(104:118,58:72) = newcheck;
